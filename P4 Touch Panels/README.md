...
To **push per-panel NVS defaults when that heartbeat arrives**, use [`home_assistant/ha_automation_hmi_defaults_from_parameters.yaml`](home_assistant/ha_automation_hmi_defaults_from_parameters.yaml): it listens on `esp_hmi/device/+/status/parameters`, derives the 12-digit hex MAC from the topic, and publishes **`cmd/set_default_screen`** (plain slug) and **`cmd/set_idle_timeout`** (JSON) per MAC. The bundled example maps `30eda0e20a9e` → default `ollie_room`, `30eda0e20a61` → `front_gate`, and sets idle return to **`ollie_room`** after **45** seconds for **both** panels—edit the YAML to change that. Extend the `choose` branches for additional devices.
