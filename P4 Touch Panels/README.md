...
On each MQTT connect the device also publishes **retained** JSON to **`esp_hmi/device/<MAC>/status/parameters`** (QoS 1) with firmware/version and identity fields (`node_id`, `mac`, `firmware_version`, `chip_target`, `chip_revision`, `ha_device_name`, `wifi_ssid` or null, `default_screen`, `idle_timeout_screen`, `idle_timeout_seconds`, `normal_brightness`, `dim_brightness`, `dim_timeout_seconds`, `screen_off_timeout_seconds`, `brightness_fade_seconds`, embedded `compile_date`/`compile_time`, etc.). Do not manually retain conflicting payloads on this topic—the device overwrites its own retained message on reconnect.
...
`home` · `ollie_room` · `ollie_room_legacy` · `front_gate` · `pipboy` · `settings` · `study` · `about` · `front_door` · `hvac` · `hvac_legacy` · `house_battery`
