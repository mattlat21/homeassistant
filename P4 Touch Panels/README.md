...
On each MQTT connect the device also publishes **retained** JSON to **`esp_hmi/device/<MAC>/status/parameters`** (QoS 1) with firmware/version and identity fields (`node_id`, `mac`, `firmware_version`, `chip_target`, `chip_revision`, `ha_device_name`, `wifi_ssid` or null, `default_screen`, `idle_timeout_screen`, `idle_timeout_seconds`, `normal_brightness`, `dim_brightness`, `dim_timeout_seconds`, `screen_off_timeout_seconds`, embedded `compile_date`/`compile_time`, etc.). Do not manually retain conflicting payloads on this topic—the device overwrites its own retained message on reconnect.
...
| `esp_hmi/device/<mac>/cmd/set_display_power` | JSON (UTF-8), **partial update OK**: optional **`normal_brightness`**, **`dim_brightness`**, **`dim_timeout_seconds`**, **`screen_off_timeout_seconds`**. Brightness 0–100; timeouts 0 disable that stage. Applied on LVGL thread; **persists** to NVS. | Controls backlight **Normal → Dim → Off** from **LVGL inactivity** (same source as idle timeout). Touch/activity restores **normal** brightness immediately. If both timeouts non-zero, **screen off ≥ dim**. |
| `esp_hmi/device/<mac>/cmd/wake_display` | Ignored (`1`, empty, …) | **Wake display**: reset LVGL inactivity (dim/off and idle countdown) and restore **normal** brightness immediately. Does not change NVS settings. |
