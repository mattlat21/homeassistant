...
3. **Settings**: Read-only summary of **firmware version** (from build-time `FW_VER_*` ints), **Wi-Fi SSID** when associated, and the **default startup screen** (friendly name + slug). Reachable from the Home launcher (not on the standard taskbar).
4. **Debug**: Live **heap / internal RAM / SPIRAM / DMA** stats plus **boot count** and **restart reason**. On the Home launcher and on the **standard taskbar** (replaces Settings on the dock). Metrics are also published to MQTT `status/memory` for Home Assistant.
5. **Ollie’s Room**: Grid with a **room-mode** selector (**Normal**, **Rest Time**, **Sleep Time**) synced to Home Assistant via MQTT (see below), **`ui_climate_control_1`** synced via **four shared retained MQTT topics** under `esp_hmi/data/bedroom3/climate/` (setpoint, current, heater, control) plus climate button payloads (see [Ollie climate](#home-assistant-ollie-climate--mqtt)), a **light** button, and a **fan** button that opens a modal to choose **Fan off / Low / Medium / High** (JSON `button` values `fan_off`, `fan_1` … `fan_3`). **All** panel outputs (buttons + room) publish on **`esp_hmi/device/<MAC>/status/button_press`** as **`{"button": "…"}`** (e.g. `light`, `Normal`, `climate_temp_dn`). Use MQTT discovery and the automations in [Ollie MQTT button_press](#home-assistant-ollie-mqtt-button_press) below.
6. **Gestures** (on app screens, not on loading): **swipe up** returns to **Home**. Left/right swipe between apps is disabled (use the launcher or taskbar).
...
On each MQTT connect the device also publishes **retained** JSON to **`esp_hmi/device/<MAC>/status/parameters`** (QoS 1) with firmware/version and identity fields (`node_id`, `mac`, `firmware_version`, `chip_target`, `chip_revision`, `ha_device_name`, `wifi_ssid` or null, `default_screen`, `idle_timeout_screen`, `idle_timeout_seconds`, `normal_brightness`, `dim_brightness`, `dim_timeout_seconds`, `screen_off_timeout_seconds`, `brightness_fade_seconds`, **`boot_count`**, **`restart_reason`**, embedded `compile_date`/`compile_time`, etc.). Do not manually retain conflicting payloads on this topic—the device overwrites its own retained message on reconnect.

While MQTT is connected the device also publishes **retained** JSON to **`esp_hmi/device/<MAC>/status/memory`** (QoS 1) on connect and every **30 s** with heap/SPIRAM metrics (`heap_free`, `heap_min_free`, `internal_free`/`internal_total`/`internal_largest`, `spiram_free`/`spiram_total`/`spiram_largest`, `dma_free`, all in bytes). This is separate from `status/parameters` so HA idle-default automations are not re-triggered by memory churn.
...
`home` · `ollie_room` · `ollie_room_legacy` · `front_gate` · `pipboy` · `settings` · `debug` · `study` · `about` · `front_door` · `hvac` · `hvac_legacy` · `house_battery`
...
| `esp_hmi/device/<mac>/cmd/set_display_power` | JSON (UTF-8), **partial update OK**: optional **`normal_brightness`**, **`dim_brightness`**, **`dim_timeout_seconds`**, **`screen_off_timeout_seconds`**, **`brightness_fade_seconds`**. Brightness 0–100; timeouts 0 disable that stage; fade 0 = instant (default 1 s). Applied on LVGL thread; **persists** to NVS. | Controls backlight **Normal → Dim → Off** from **LVGL inactivity** (same source as idle timeout). Transitions fade over **`brightness_fade_seconds`**. Touch/activity restores **normal** brightness immediately. If both timeouts non-zero, **screen off ≥ dim**. |
| `esp_hmi/device/<mac>/cmd/wake_display` | Ignored (`1`, empty, …) | **Wake display**: reset LVGL inactivity (dim/off and idle countdown) and fade to **normal** brightness. Does not change NVS settings. |
...
mosquitto_pub -h <broker> -t 'esp_hmi/device/<mac>/cmd/set_display_power' \
  -m '{"normal_brightness":100,"dim_brightness":15,"dim_timeout_seconds":60,"screen_off_timeout_seconds":300}' -q 1
```
...
```bash
mosquitto_pub -h <broker> -t 'esp_hmi/device/aabbccddeeff/cmd/ota_update' -m '{"url":"http://latimer.net/ha/fware/esphmi/v1.1.4/esp_hmi.bin","version":"1.1.4"}' -q 1
```

**Progress (panel → HA):** While OTA runs, the panel publishes JSON on **`esp_hmi/device/<mac>/status/ota_progress`** (QoS 1, not retained), e.g. `{"state":"downloading","percent":42,"version":"1.1.4"}`. States: `idle`, `starting`, `downloading`, `verifying`, `success`, `failed` (optional `error`). The **ESP HMI Panels** integration exposes this as the **OTA update progress** sensor; use **Firmware version** + **Install firmware** on the device to trigger an update.
...
data:
  ...
  payload: '{"url":"http://latimer.net/ha/fware/esphmi/v1.1.4/esp_hmi.bin","version":"1.1.4"}'
  qos: 1
```
