...
/** Launcher: 4×4 cell grid; tiles placed row-major (About and Settings last). */
...
lv_obj_t *screen_home_create(lv_display_t *disp)
{
    ...
    const int32_t gap = ui_layout_grid_gap_for_screen(HOME_LAUNCHER_TILE_EDGE_PX, HOME_LAUNCHER_TILE_EDGE_PX,
    ...
    for (unsigned i = 0; i < (unsigned)(sizeof(tiles) / sizeof(tiles[0])); i++) {
        ...
        int32_t x, y;
        if (!ui_layout_grid_rc_to_pos(gap, HOME_LAUNCHER_TILE_EDGE_PX, (uint8_t)HOME_GRID_N, row, col, &x, &y)) {
            continue;
        }
